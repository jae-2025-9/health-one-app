...
L1 코어/인증 레인의 동결 기준은 [L1 동결 계약](FROZEN_CONTRACTS.md)과 [L1 API 슬라이스](api/l1-core-auth.md)에 정리했습니다. L2-L4는 enum, 코어 테이블, 응답 봉투, `user_id`/`health_event_id` FK 규약을 이 기준으로 참조합니다.
